...
export class ValidationHelpers {
  ...
  static validateCSVData(data) {
    ...
    // Detect if data is already in parsed format (has options array) vs raw CSV row format
    const firstRow = data[0];
    const isParsedFormat = firstRow.options && Array.isArray(firstRow.options);
    
    if (isParsedFormat) {
      // Data is already parsed by EnhancedCSVManager - validate parsed questions
      data.forEach((question, index) => {
        ...
        if (question.type === 'multiple_choice' || !question.type) {
          if (!question.options || question.options.length < 2) {
            rowErrors.push(`Row ${index + 1}: Multiple choice question requires at least 2 options`);
          }
        }
      ...
      // Legacy: Raw CSV row format - use original header-based validation
      ...
      else missingFields.push('Correct Answer');
